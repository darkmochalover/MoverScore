...
def get_bert_embedding(all_sens, model, tokenizer, idf_dict,
                       batch_size=-1,device=get_device()):
...
def word_mover_score(refs, hyps, idf_dict_ref, idf_dict_hyp, stop_words=[], n_gram=1, remove_subwords = True, batch_size=256,device=get_device()):
    preds = []
    for batch_start in range(0, len(refs), batch_size):
        ...
        distance_matrix = batched_cdist_l2(raw, raw).float().cpu().numpy()
        ...
        for i in range(batch_size):  
            ...
            dst = distance_matrix[i].astype(dtype=np.float64)
